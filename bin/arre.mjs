...
const scriptIndex = args.findIndex((x) => arreActions.indexOf(x) >= 0)
...
if (arreActions.includes(script)) {
  ...
  const call = nodeArgs.concat(arreScript).concat(arreArgs)

  const result = spawn.sync(process.execPath, call, { stdio: "inherit" })
  if (result.signal) {
