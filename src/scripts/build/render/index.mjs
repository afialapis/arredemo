...
const _renderDemoJs = (source, dest, _pkgPath, _pkgJson, arreConfig, _readmes) => {
  let js = fs.readFileSync(source, { encoding: "utf8", flag: "r" })
...
const renderArreDemoApp = (pkgPath, pkgJson, arreConfig, readmes) => {
  ...
  //  except index.html.mustache, wich already goes to pkg/arredemo/index.html
  copyFolderSync(tmplFolder, outFolder, (sourceEl, destEl) => {
    if (["index.html.mustache"].indexOf(path.basename(destEl)) >= 0) {
      return () =>
        renderIndexHtml(
          sourceEl,
          destEl.replace("index.html.mustache", "index.html").replace(`${path.sep}_temp`, ""),
          pkgJson,
          arreConfig
        )
    }
